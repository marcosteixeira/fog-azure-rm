...
class TestRecordSet < Minitest::Test
  ...
  def test_save_method_response
    @service.stub :create_or_update_record_set, @response do
      assert_instance_of Fog::AzureRM::DNS::RecordSet, @record_set.save
    end
    record_set_cname = record_set_cname(@service)
    response = ApiStub::Models::DNS::RecordSet.response_for_cname(@dns_client1)
    @service.stub :create_or_update_record_set, response do
      assert_instance_of Fog::AzureRM::DNS::RecordSet, record_set_cname.save
    end
  end
  ...
  def test_update_ttl_response
    @service.stub :create_or_update_record_set, @response do
      assert_instance_of Fog::AzureRM::DNS::RecordSet, @record_set.update_ttl(70)
  ...
  def test_add_a_type_record_response
    @service.stub :create_or_update_record_set, @response do
      assert_instance_of Fog::AzureRM::DNS::RecordSet, @record_set.add_a_type_record('5.3.2.1')
    end
  end

  def test_remove_a_type_record_response
    @service.stub :create_or_update_record_set, @response do
      assert_instance_of Fog::AzureRM::DNS::RecordSet, @record_set.remove_a_type_record('5.3.2.1')
    end
  end
end
