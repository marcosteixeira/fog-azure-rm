...
class TestSqlServers < Minitest::Test
  ...
  def test_all_method_response
    @service.stub :list_sql_servers, [@sql_server_response] do
      assert_instance_of Fog::AzureRM::Sql::SqlServers, @sql_servers.all
      assert @sql_servers.all.size >= 1
      @sql_servers.all.each do |s|
        assert_instance_of Fog::AzureRM::Sql::SqlServer, s
      end
    end
  end
  ...
  def test_get_method_response
    @service.stub :get_sql_server, @sql_server_response do
      assert_instance_of Fog::AzureRM::Sql::SqlServer, @sql_servers.get('fog-test-rg', 'fog-test-server-name')
    end
  end
end
