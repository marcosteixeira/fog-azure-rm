...
class TestFirewallRules < Minitest::Test
  ...
  def test_all_method_response
    ...
    @service.stub :list_firewall_rules, list_firewall_rule_response do
      assert_instance_of Fog::AzureRM::Sql::FirewallRules, @firewall_rules.all
      assert @firewall_rules.all.size >= 1
      @firewall_rules.all.each do |s|
        assert_instance_of Fog::AzureRM::Sql::FirewallRule, s
      end
    end
  end
  ...
  def test_get_method_response
    @service.stub :get_firewall_rule, @create_firewall_rule_response do
      assert_instance_of Fog::AzureRM::Sql::FirewallRule, @firewall_rules.get('fog-test-rg', 'fog-test-server-name', 'fog-test-rule-name')
    end
  end
end
