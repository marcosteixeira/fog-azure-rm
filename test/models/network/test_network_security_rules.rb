...
class TestNetworkSecurityRules < Minitest::Test
  def setup
    ...
    @network_security_rules = Fog::AzureRM::Network::NetworkSecurityRules.new(resource_group: 'fog-test-rg', network_security_group_name: 'fog-test-nsg', service: @service)
  ...
  def test_all_method_response
    @service.stub :list_network_security_rules, @response do
      assert_instance_of Fog::AzureRM::Network::NetworkSecurityRules, @network_security_rules.all
      assert @network_security_rules.all.size >= 1
      @network_security_rules.all.each do |nsr|
        assert_instance_of Fog::AzureRM::Network::NetworkSecurityRule, nsr
      end
    end
  end
  ...
  def test_get_method_response
    @service.stub :get_network_security_rule, @response[0] do
      assert_instance_of Fog::AzureRM::Network::NetworkSecurityRule, @network_security_rules.get('fog-test-rg', 'fog-test-nsg', 'fog-test-nsr')
    end
  end
