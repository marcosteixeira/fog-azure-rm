...
class TestVirtualNetworkGatewayConnections < Minitest::Test
  def setup
    ...
    @gateway_connections = Fog::AzureRM::Network::VirtualNetworkGatewayConnections.new(resource_group: 'fog-rg', service: @service)
  ...
  def test_all_method_response
    ...
    @service.stub :list_virtual_network_gateway_connections, connection_response do
      assert_instance_of Fog::AzureRM::Network::VirtualNetworkGatewayConnections, @gateway_connections.all
      assert @gateway_connections.all.size >= 1
      @gateway_connections.all.each do |gateway_connection|
        assert_instance_of Fog::AzureRM::Network::VirtualNetworkGatewayConnection, gateway_connection
      end
    end
  end
  ...
  def test_get_method_response
    @service.stub :get_virtual_network_gateway_connection, @response do
      assert_instance_of Fog::AzureRM::Network::VirtualNetworkGatewayConnection, @gateway_connections.get('fog-rg', 'cn1')
    end
  end
