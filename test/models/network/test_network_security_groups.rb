...
class TestNetworkSecurityGroups < Minitest::Test
  def setup
    ...
    @network_security_groups = Fog::AzureRM::Network::NetworkSecurityGroups.new(resource_group: 'fog-test-rg', service: @service)
  ...
  def test_all_method_response
    @service.stub :list_network_security_groups, @response do
      assert_instance_of Fog::AzureRM::Network::NetworkSecurityGroups, @network_security_groups.all
      assert @network_security_groups.all.size >= 1
      @network_security_groups.all.each do |nsg|
        assert_instance_of Fog::AzureRM::Network::NetworkSecurityGroup, nsg
      end
    end
  end
  ...
  def test_get_method_response
    @service.stub :get_network_security_group, @response[0] do
      assert_instance_of Fog::AzureRM::Network::NetworkSecurityGroup, @network_security_groups.get('fog-test-rg', 'fog-test-nsg')
    end
  end
