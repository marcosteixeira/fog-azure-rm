...
class TestPublicIps < Minitest::Test
  def setup
    ...
    @public_ips = Fog::AzureRM::Network::PublicIps.new(resource_group: 'fog-test-rg', service: @service)
  ...
  def test_all_method_response
    @service.stub :list_public_ips, @response do
      assert_instance_of Fog::AzureRM::Network::PublicIps, @public_ips.all
      assert @public_ips.all.size >= 1
      @public_ips.all.each do |pip|
        assert_instance_of Fog::AzureRM::Network::PublicIp, pip
      end
    end
  end
  ...
  def test_get_method_response
    ...
    @service.stub :get_public_ip, response do
      assert_instance_of Fog::AzureRM::Network::PublicIp, @public_ips.get('fog-test-rg', 'fog-test-public-ip')
    end
  end
