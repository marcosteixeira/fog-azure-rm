...
class TestLoadBalancers < Minitest::Test
  def setup
    ...
    @load_balancers = Fog::AzureRM::Network::LoadBalancers.new(resource_group: 'fog-test-rg', service: @service)
  ...
  def test_all_method_response
    ...
    @service.stub :list_load_balancers, response do
      assert_instance_of Fog::AzureRM::Network::LoadBalancers, @load_balancers.all
      assert @load_balancers.all.size >= 1
      @load_balancers.all.each do |lb|
        assert_instance_of Fog::AzureRM::Network::LoadBalancer, lb
      end
    end
  end
  ...
  def test_get_method_response
    @service.stub :get_load_balancer, @response do
      assert_instance_of Fog::AzureRM::Network::LoadBalancer, @load_balancers.get('fog-test-rg', 'mylb1')
    end
  end
