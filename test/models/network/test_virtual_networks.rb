...
class TestVirtualNetworks < Minitest::Test
  def setup
    ...
    @virtual_networks = Fog::AzureRM::Network::VirtualNetworks.new(resource_group: 'fog-test-rg', service: @service)
  ...
  def test_all_method_response_for_rg
    ...
    @service.stub :list_virtual_networks, response do
      assert_instance_of Fog::AzureRM::Network::VirtualNetworks, @virtual_networks.all
      assert @virtual_networks.all.size >= 1
      @virtual_networks.all.each do |nic|
        assert_instance_of Fog::AzureRM::Network::VirtualNetwork, nic
      end
    end
  end

  def test_all_method_response
    virtual_networks = Fog::AzureRM::Network::VirtualNetworks.new(service: @service)
    response = [ApiStub::Models::Network::VirtualNetwork.create_virtual_network_response(@network_client)]
  ...
  def test_get_method_response
    ...
    @service.stub :get_virtual_network, response do
      assert_instance_of Fog::AzureRM::Network::VirtualNetwork, @virtual_networks.get('fog-rg', 'fog-test-virtual-network')
    end
  end
