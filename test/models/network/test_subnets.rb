...
class TestSubnets < Minitest::Test
  def setup
    ...
    @subnets = Fog::AzureRM::Network::Subnets.new(resource_group: 'fog-test-rg', virtual_network_name: 'fog-test-virtual-network', service: @service)
  ...
  def test_all_method_response
    ...
    @service.stub :list_subnets, response do
      assert_instance_of Fog::AzureRM::Network::Subnets, @subnets.all
      assert @subnets.all.size >= 1
      @subnets.all.each do |subnet|
        assert_instance_of Fog::AzureRM::Network::Subnet, subnet
      end
    end
  end
  ...
  def test_get_method_response
    ...
    @service.stub :get_subnet, response do
      assert_instance_of Fog::AzureRM::Network::Subnet, @subnets.get('fog-test-rg', 'fog-test-vnet', 'fog-test-subnet')
    end
  end
