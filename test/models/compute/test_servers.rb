...
class TestServers < Minitest::Test
  def setup
    ...
    @servers = Fog::AzureRM::Compute::Servers.new(resource_group: 'fog-test-rg', service: @service)
  ...
  def test_all_method_response
    ...
    @service.stub :list_virtual_machines, response do
      assert_instance_of Fog::AzureRM::Compute::Servers, @servers.all
      assert @servers.all.size >= 1
      @servers.all.each do |s|
        assert_instance_of Fog::AzureRM::Compute::Server, s
      end
    end
  end
  ...
  def test_get_method_response
    @service.stub :get_virtual_machine, @response do
      assert_instance_of Fog::AzureRM::Compute::Server, @servers.get('fog-test-rg', 'fog-test-server')
    end
  end
