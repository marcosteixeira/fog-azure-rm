...
class TestGateway < Minitest::Test
  def setup
    ...
    gateways = Fog::AzureRM::ApplicationGateway::Gateways.new(resource_group: 'fog-test-rg', service: @service)
  ...
  def test_save_method_response
    @service.stub :create_or_update_application_gateway, @response do
      assert_instance_of Fog::AzureRM::ApplicationGateway::Gateway, @gateway.save
    end
  end

  def test_update_sku
    @service.stub :update_sku_attributes, @response do
      assert_instance_of Fog::AzureRM::ApplicationGateway::Gateway, @gateway_obj.update_sku('Standard_Medium', '2')
    end
  end

  def test_update_gateway_ip_configuration
    @service.stub :update_subnet_id_in_gateway_ip_configuration, @response do
      assert_instance_of Fog::AzureRM::ApplicationGateway::Gateway, @gateway_obj.update_gateway_ip_configuration('/subscriptions/{guid}/resourceGroups/{resourceGroupName}/providers/Microsoft.Network/virtualNetworks/vnet/subnets/GatewaySubnet')
    end
  end

  def test_add_ssl_certificate
    @service.stub :create_or_update_application_gateway, @response do
      assert_instance_of Fog::AzureRM::ApplicationGateway::Gateway, @gateway_obj.add_ssl_certificate(@ssl_certifcate)
    end
  end

  def test_remove_ssl_certificate
    @service.stub :create_or_update_application_gateway, @response do
      assert_instance_of Fog::AzureRM::ApplicationGateway::Gateway, @gateway_obj.remove_ssl_certificate(@ssl_certifcate)
    end
  end

  def test_add_frontend_port
    @service.stub :create_or_update_application_gateway, @response do
      assert_instance_of Fog::AzureRM::ApplicationGateway::Gateway, @gateway_obj.add_frontend_port(@frontend_port)
    end
  end

  def test_remove_frontend_port
    @service.stub :create_or_update_application_gateway, @response do
      assert_instance_of Fog::AzureRM::ApplicationGateway::Gateway, @gateway_obj.remove_frontend_port(@frontend_port)
    end
  end

  def test_add_probe
    @service.stub :create_or_update_application_gateway, @response do
      assert_instance_of Fog::AzureRM::ApplicationGateway::Gateway, @gateway_obj.add_probe(@probe)
    end
  end

  def test_remove_probe
    @service.stub :create_or_update_application_gateway, @response do
      assert_instance_of Fog::AzureRM::ApplicationGateway::Gateway, @gateway_obj.remove_probe(@probe)
    end
  end
