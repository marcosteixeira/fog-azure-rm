    ...
    class Compute < Fog::Service
      ...
      class Real
        def initialize(options)
          ...
          @storage_service = Fog::AzureRM::Storage.new(tenant_id: options[:tenant_id], client_id: options[:client_id], client_secret: options[:client_secret], subscription_id: options[:subscription_id], environment: options[:environment])
        end
      end
    end
  end
