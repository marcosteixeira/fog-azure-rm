  ...
  module AzureRM
    class Network
      ...
      class ExpressRouteCircuitAuthorizations < Fog::Collection
        model Fog::AzureRM::Network::ExpressRouteCircuitAuthorization
        attribute :resource_group
        attribute :circuit_name

        def all
          requires :resource_group, :circuit_name
          circuit_authorizations = service.list_express_route_circuit_authorizations(resource_group, circuit_name).map { |circuit_authorization| Fog::AzureRM::Network::ExpressRouteCircuitAuthorization.parse(circuit_authorization) }
        ...
        def get(resource_group_name, circuit_name, authorization_name)
          ...
          express_route_circuit_authorization = Fog::AzureRM::Network::ExpressRouteCircuitAuthorization.new(service: service)
          express_route_circuit_authorization.merge_attributes(Fog::AzureRM::Network::ExpressRouteCircuitAuthorization.parse(circuit_authorization))
        end
